...
const IdNotaRodapeAttribute = new Parchment.Attributor.Attribute('id-lexml-emenda-nota-rodape', 'id-lexml-emenda-nota-rodape', cfgInline);
...
class NotaRodapeBlot extends Embed {
  static create(value) {
    ...
    node.setAttribute('class', 'lexml-emenda-nota-rodape');
  ...
  //   return { 'lexml-emenda-nota-rodape': domNode.notaRodape || NotaRodapeBlot.buildNotaRodape(domNode) };
  // }

  static buildNotaRodape(domNode) {
    return {
      // id: domNode.getAttribute('id'),
      id: domNode.getAttribute('id-lexml-emenda-nota-rodape'),
  ...
  static valueToAttributes(value, domNode) {
    ...
    value.id && domNode.setAttribute('id-lexml-emenda-nota-rodape', value.id);
...
NotaRodapeBlot.blotName = 'lexml-emenda-nota-rodape';
NotaRodapeBlot.tagName = 'lexml-emenda-nota-rodape';
...
class ModuloNotaRodape extends Module {
  ...
  constructor(quill, options) {
    ...
    if (toolbar) {
      toolbar.addHandler('lexml-emenda-nota-rodape', this.solicitarTexto.bind(this));
    }
  ...
  addClipboardMatcher() {
    this.quill.clipboard.addMatcher('lexml-emenda-nota-rodape', (node, delta) => {
      let match = Parchment.query(node);
      if (match == null || match.blotName !== 'lexml-emenda-nota-rodape') {
        return delta;
      }

      const id = this.isAbrindoTexto ? node.getAttribute('id-lexml-emenda-nota-rodape') : this.gerarId();
      ...
      return new Delta().insert({ 'lexml-emenda-nota-rodape': notaRodape });

      // const ops = delta.ops.reduce((acc, op) => {
      //   if (op.insert && op.attributes?.['id-lexml-emenda-nota-rodape']) {
      //     const { 'id-lexml-emenda-nota-rodape': id, numero, texto } = op.attributes || {};
      //     const notaRodape = new NotaRodape({ id, numero: +numero, texto });
      //     acc.push({ insert: { 'lexml-emenda-nota-rodape': notaRodape } });
      //   }
      //   return acc;
      // }, []);
  ...
  hasNotaRodape(delta) {
    return delta?.ops?.find(op => op.insert?.['lexml-emenda-nota-rodape']);
  }

  onClick(e) {
    const el = e.target;
    const elRev = el?.closest('ins, del');
    if (!elRev && (el?.tagName === 'LEXML-EMENDA-NOTA-RODAPE' || el?.parentElement?.tagName === 'LEXML-EMENDA-NOTA-RODAPE')) {
      e.preventDefault();
      e.stopPropagation();
  ...
  atualizarTexto(notaRodape, novoTexto) {
    ...
    blot.format('lexml-emenda-nota-rodape', { ...notaRodape, texto: novoTexto });
  ...
  adicionar(texto) {
    ...
    const delta = new Delta().retain(range.index).delete(range.length).insert({ 'lexml-emenda-nota-rodape': notaRodape });
  ...
  findNodeById(id) {
    return this.quill.root.querySelector(`lexml-emenda-nota-rodape[id-lexml-emenda-nota-rodape="${id}"]`);
  }

  findBlotsNotaRodape() {
    return Array.from(this.quill.root.querySelectorAll('lexml-emenda-nota-rodape')).map(domNode => {
  ...
  ajustarConteudoTagsNotaRodape(html) {
    // Ajusta o conteúdo das tags <lexml-emenda-nota-rodape> para que o número da nota fique dentro da tag <lexml-emenda-nota-rodape>
    return html.replace(/<lexml-emenda-nota-rodape.+?<\/lexml-emenda-nota-rodape>/g, (texto: string) => texto.replace(/>.?<span[^>]*>(\d+)<\/span>.?</g, '>$1<'));
  }
}
