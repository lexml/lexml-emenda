...
export const aplicaAlteracoesEmenda = (state: any, action: any): State => {
  ...
  if (action.alteracoesEmenda.dispositivosSuprimidos?.length) {
  ...
  if (action.alteracoesEmenda.dispositivosModificados?.length) {
  ...
  if (action.alteracoesEmenda.dispositivosAdicionados?.length) {
...
const criaEventosParaDispositivoAgrupador = (state: any, dea: DispositivoEmendaAdicionado): StateEvent[] => {
  ...
  if (ref) {
    ...
    ajustaAtributosDispositivoAdicionado(novo, dea, state.modo);
