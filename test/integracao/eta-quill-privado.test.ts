...
describe('Núcleo EtaQuill com Quill privado', () => {
  ...
  it('configura o EtaQuill sem alterar o registry da aplicação', () => {
    ...
    expect(registryDaAplicacao['modules/clipboard']).to.equal(Quill.import('modules/clipboard'));
    ...
    expect(PrivateQuill.import('modules/eta-clipboard')).to.equal(EtaClipboard);
    expect(PrivateQuill.import('modules/eta-keyboard')).to.equal(EtaKeyboard);
    ...
    expect(PrivateQuill.import('formats/EtaBlotConteudo')).to.equal(EtaQuill.import('formats/EtaBlotConteudo'));
  });

  it('mantém a mesma configuração quando chamado novamente', () => {
    configurePrivateQuill();
    ...
    expect((PrivateQuill as any).imports).to.equal(registry);
    expect(Object.keys(registry)).to.deep.equal([...registrosAntes.keys()]);
    registrosAntes.forEach((definition, path) => expect(registry[path]).to.equal(definition));
  });

  it('mantém editores estruturado e rico com módulos diferentes simultaneamente', () => {
    configurePrivateQuill();
    const registry = (PrivateQuill as any).imports;
    const keyboardPadrao = PrivateQuill.import('modules/keyboard');
    const clipboardPadrao = PrivateQuill.import('modules/clipboard');
    ...
    try {
      ...
      expect((PrivateQuill as any).imports).to.equal(registry);
      expect(PrivateQuill.import('modules/keyboard')).to.equal(keyboardPadrao);
      expect(PrivateQuill.import('modules/clipboard')).to.equal(clipboardPadrao);
    } finally {
      editorHtml.remove();
      bufferHtml.remove();
      ricoHtml.remove();
